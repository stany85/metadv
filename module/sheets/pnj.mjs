 ...
 * @extends {ActorSheet}
 */
export class MetadvPNJ extends ActorSheet {

  /** @override */
  static get defaultOptions() {
    return mergeObject(super.defaultOptions, {
      classes: ["metadv", "sheet", "actor"],
      template: "systems/metadv/templates/actor/actor-PNJ-sheet.hbs",
      width: 575,
      height: 710,
...
async _onDropItemCreate(itemData, source, shiftKey) {
  ...
  // Evenement lancer dés
    activateListeners(html){
      ...
      //html.find('.Init').click(this._onRollInit.bind(this));  
      ...
      //
      //html.find('.moinsDeSpecial').click(this._onMoinsSpecial.bind(this)); 
      //html.find('.plusDeSpecial').click(this._onPlusSpecial.bind(this));
      //html.find('.cocheMun').click(this._onCocheMun.bind(this));
      //html.find('.plusMun').click(this._onPlusMun.bind(this));  
      //html.find('.moinsMun').click(this._onMoinsMun.bind(this));
    ...
    html.find('.useweapon').click((ev) => {
      ...
      console.log(Arme.system.AttributAssocie + " -" + NomArme+ " - "+ValeurDe);
  ...
  {    
    ...
    var IntituleComp = "<div class='infoJetComp'>"+Intitule+" : "+NbDes+" dés</div>";
    //On vérifie les dés bonus en y ajoutant l'ititulé
    if(NbDesBonus>0)
        {
          var desb = "dé";
          if(NbDesBonus>1){desb ="dés";}
        IntituleComp += "<div class='chat-bonus'> + "+NbDesBonus+ " "+desb+" bonus</div> ";  
        }
    ...
    if(event.target.dataset["dice"]>0)
        ...
        const NbDesCompetence = DesTotal+typeDe;  
          ...
          /////////////////////////////////////////////
            ...
            if(this.actor.system.JetDe=="td" &&  TypeJet=="n")
                ...
                var typejet ="<div class='red'>Très difficile</div>";
                }
          ...
          if(TypeJet=="sc")
                {
                InfoJet = "<div class='infojet'>SECONDE CHANCE</div>";  
                }
          if(TypeJet=="pmf")
                {
                InfoJet = "<div class='infojet'>PEUT MIEUX FAIRE</div>";  
                }
          if(TypeJet=="e2f")
                {
                InfoJet = "<div class='infojet'>ESSAIE ENCORE UNE FOIS</div>";  
                }
        ...
        {
        if(this.actor.system.Mutant==false){NbDesMf=NbDesMf-1;}
    ...
    if(NbDesBonus!=0)
        {
          var desb = "dé";
          if(NbDesBonus>1){desb ="dés";}
          if(NbDesBonus>0){IntituleComp += "<div class='chat-bonus'>+ "+NbDesBonus+ " "+desb+" bonus</div> "; }
          else{IntituleComp += "<div class='chat-bonus'>"+NbDesBonus+ " "+desb+" bonus</div> ";}
